...
We envisioned a sentiment analysis comparison of the New York Times movie reviews and Tweets of the same movies. Our hope is that we would be able to answer how favorably did the New York Times review a given movie compared to the average sentiment on Twitter. 

Hopefully, this approach will give us insight on market research analysis on social media and traditional media sources. Our resulting analysis should be helpful to moviegoers and the movie industry in understanding the perception of reviews.  
...
Frome these results, it did not appear that there was a relationship between these two variables.
...
```{r, eval = TRUE, warning=FALSE, message=FALSE, fig.width=10, fig.height=8}
...
We conclucded that there is no correlation between the New York Times review and the average sentiment analysis about a given movie.  
...
The results indicate that the New York Times does not reflect the tweet scores which we used as a proxy indicator of public opinion of the movies. Further research is needed to stratify the tweets because several movies are intended for different demographics and a more focused approach may yield different results. One of the challenges of this type of analysis was our inability to understand if we had an overlap in people reading the New York Times and then tweeting about the film. An experiment would needed to be conducted to be able to conclude a causal relationship between traditional media sources influencing new social media sources. 
...
![](https://raw.githubusercontent.com/spsstudent15/Data607FinalProject/master/cuny607.PNG)
