...
The question when conducting sentiment analysis is how confident are we on the results.  Based on this quick analysis, we can see that the sentiment analysis score does not always matched with whether the movie was recommended.  

We number of false negative are really indicators of how difficult it is to accurately measure the tonality and sentiment in a document.  There are two movies that fell into the category of false negative.  The movies were recommended by the NY Times critic however the sentiment analysis score was less than the threshold we considered.  Upon reading the reviews, for these movies "Viva" and "A Hologram for a King", it is understandable that the results of the Sentiment Analysis are as they are.  Both movies are dramas with elements of the review depicting difficult, dark, and negative terms.  

The False positive results are more easily explained, the NY Times can give neutral to positive review without recommending the movies.  Also, score of 0.75 may be too low a limit to be expected to recommendation.  We will therefore consider score below .50 as negative, score between .50 and .80 as neutral and above .80 as positive.  

We will rerun the analysis based on these new limits.  False negative will be scores < .50 but with recommendation, False positive will be scores >= 80 without recommendation, and False neutral will be scores within interval [50;80) with recommendation;  
...
Percentage of False Positive: `r scales::percent(nrow(nyt_false_positive)/nrow(movie_score_nyt))`    
Percentage of False Negative: `r scales::percent(nrow(nyt_false_negative)/nrow(movie_score_nyt))`   
Percentage of False Neutral : `r scales::percent(nrow(nyt_false_neutral)/nrow(movie_score_nyt))`   
Percentage of True Positive : `r scales::percent(nrow(nyt_true_positive)/nrow(movie_score_nyt))`   
Percentage of True Neutral  : `r scales::percent(nrow(nyt_true_neutral)/nrow(movie_score_nyt))`
Percentage of True Negative : `r scales::percent(nrow(nyt_true_negative)/nrow(movie_score_nyt))`   
...
One of the question that we would like answer is how relevant is the NY Times review as a predictor of the Sentiment Analysis for the tweets.  In order to attempt to answer this question will will compare the NY Times score against the averate of the sentiments analysis for each movie.

### Data Set  

For this analysis, we will consider the average of the sentiment analysis for tweets and the NY Times sentiment analsyis.  However, we will filter it to consider only movies for which we have at least 15 tweets.  
...
We will consider the NY Times Review Score and the average Sentiment Analyis score of the tweets for a given movie.  both these variables are numerical in nature.  we will first plot them.  
...
From the scatter plot, it is does not seem probable that there is relation between these two variables.  We will now fit a line through the graph and calculate the linear regression.
...
Frome these results, it does not appear that there is relation between the two variables.  

To have a better picture of the variations between the average of the tweets sentiment analysis and the NY Times review score will plot the confidence interval (95%) for the tweets for each movie concurrently and compare.  We will also place on the graph the NY Times review score. 
...
The movie are sorted by number of overall tweets in descending order.  

From this graph, we have only one movie for whom the NY Times score falls into the confidence interval around the average of the tweets: "Viva", we will recall that this is the movie that is a NY Critics' Pick that had a poor result on the sentiment analysis of the review.  

We can conclude that there is no correlation between the NY Times review and the average sentiment analysis about a given movie.  
...
This was our first experience contributing to a repository using GitHub. It made sharing the code and version control of each change much easier than previous attempts. Github versioning control is powerful and can handle most merge seamlessly.  However if there is a conflic that Github cannot handle, the resolution must be handle through git command line and can be challenging.  
...
Twitter free service for directed search limits the number of tweets to the last 5000.  We had to revise our approach and coordinate the timing of the NY Times reviews with when we were pulling tweets to have any hope of having relevant data. 
